...
const Search = ({
    ...
    const [showProducts, setShowProducts] = useState(search_products);
    ...
    useEffect(() => {
        if (search_products && search_products.length > 0) {
            setShowProducts(search_products)
        } else {
            setShowProducts([])
        }
    },[search_products])
    ...
    useEffect(() => {
        ...
        if (ifFiltered) {
            setShowProducts(
                Array.isArray(filtered_products) && filtered_products.length
                ? filtered_products
                : []
            )
        }
    }, [filtered_products, ifFiltered]);
    ...
    const paginatedProducts = Array.isArray(showProducts) 
            ? showProducts.slice(indexOfFirstItem, indexOfLastItem) 
            : [];
